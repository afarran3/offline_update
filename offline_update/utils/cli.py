...
def print_bench_version(ctx, param, value):
	...
	click.echo(offline_update.VERSION)
	ctx.exit()
...
def use_experimental_feature(ctx, param, value):
	...
	if value == "dynamic-feed":
		...
		offline_update.cli.dynamic_feed = True
		offline_update.cli.verbose = True
	else:
		from offline_update.exceptions import FeatureDoesNotExistError

		raise FeatureDoesNotExistError(f"Feature {value} does not exist")

	from offline_update.cli import is_envvar_warn_set
