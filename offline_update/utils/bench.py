...
from offline_update.exceptions import PatchError, ValidationError
from offline_update.utils import exec_cmd, get_bench_name, get_cmd_output, log, which

logger = logging.getLogger(offline_update.PROJECT_NAME)


@lru_cache(maxsize=None)
def get_env_cmd(cmd: str, bench_path: str = get_bench_path()) -> str:
...
def update_node_packages(bench_path=get_bench_path(), apps=None):
	print("Updating node packages...")
	from distutils.version import LooseVersion

	from offline_update.utils.app import get_develop_version
...
def install_python_dev_dependencies(bench_path=get_bench_path(), apps=None, verbose=False):
...
def update_yarn_packages(bench_path=get_bench_path(), apps=None):
	from offline_update.bench import Bench
...
def validate_upgrade(from_ver, to_ver, bench_path=get_bench_path()):
	if to_ver >= 6 and not which("npm") and not which("node") and not which("nodejs"):
		raise Exception("Please install nodejs and npm")


def post_upgrade(from_ver, to_ver, bench_path=get_bench_path()):
	from offline_update.bench import Bench
	from offline_update.config import redis
	from offline_update.config.nginx import make_nginx_conf
	from offline_update.config.supervisor import generate_supervisor_config
...
def patch_sites(bench_path=get_bench_path()):
	from offline_update.bench import Bench
	from offline_update.utils.system import migrate_site
...
def restart_supervisor_processes(bench_path=get_bench_path(), web_workers=False, _raise=False):
	from offline_update.bench import Bench
...
def restart_systemd_processes(bench_path=get_bench_path(), web_workers=False, _raise=True):
	bench_name = get_bench_name(bench_path)
	exec_cmd(
...
def restart_process_manager(bench_path=get_bench_path(), web_workers=False):
...
def build_assets(bench_path=get_bench_path(), app=None):
	command = "bench build"
	if app:
		command += f" --app {app}"
...
def handle_version_upgrade(version_upgrade, bench_path, force, reset, conf):
	from offline_update.utils import log, pause_exec
...
def update(
	...
	from offline_update.app import pull_apps
	from offline_update.bench import Bench
	from offline_update.config.common_site_config import update_config
	from offline_update.exceptions import CannotUpdateReleaseBench
	from offline_update.utils.app import is_version_upgrade
	from offline_update.utils.system import backup_all_sites

	bench_path = os.path.abspath(get_bench_path())
	bench = Bench(bench_path)
...
def clone_apps_from(bench_path, clone_from, update_app=True):
	from offline_update.app import install_app
...
def remove_backups_crontab(bench_path=get_bench_path()):
...
def set_mariadb_host(host, bench_path=get_bench_path()):
	update_common_site_config({"db_host": host}, bench_path=bench_path)


def set_redis_cache_host(host, bench_path=get_bench_path()):
	update_common_site_config({"redis_cache": f"redis://{host}"}, bench_path=bench_path)


def set_redis_queue_host(host, bench_path=get_bench_path()):
	update_common_site_config({"redis_queue": f"redis://{host}"}, bench_path=bench_path)


def set_redis_socketio_host(host, bench_path=get_bench_path()):
	update_common_site_config({"redis_socketio": f"redis://{host}"}, bench_path=bench_path)


def update_common_site_config(ddict, bench_path=get_bench_path()):
...
def validate_app_installed_on_sites(app, bench_path=get_bench_path()):
...
def check_app_installed(app, bench_path=get_bench_path()):
	try:
...
def check_app_installed_legacy(app, bench_path=get_bench_path()):
...
def validate_branch():
	from offline_update.bench import Bench
	from offline_update.utils.app import get_current_branch

	apps = Bench(get_bench_path()).apps
