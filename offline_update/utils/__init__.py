...
from offline_update import PROJECT_NAME, VERSION
...
def log(message, level=0, no_log=False, stderr=False):
	...
	if offline_update.cli.from_command_line and offline_update.cli.dynamic_feed:
...
def exec_cmd(cmd, cwd=get_bench_path(), env=None, _raise=True):
	if env:
		env.update(os.environ.copy())

	click.secho(f"$ {cmd}", fg="bright_black")

	cwd_info = f"cd {cwd} && " if cwd != get_bench_path() else ""
...
def setup_logging(bench_path=get_bench_path()) -> logging.Logger:
	LOG_LEVEL = 15
...
def get_cmd_output(cmd, cwd=get_bench_path(), _raise=True):
	output = ""
	try:
...
def run_frappe_cmd(*args, **kwargs):
	from offline_update.cli import from_command_line
	from offline_update.utils.bench import get_env_cmd

	bench_path = kwargs.get("bench_path", get_bench_path())
...
def set_git_remote_url(git_url, bench_path=get_bench_path()):
...
def run_playbook(playbook_name, extra_vars=None, tag=None):
	...
	subprocess.check_call(args, cwd=os.path.join(offline_update.__path__[0], "playbooks"))
...
def get_env_frappe_commands(bench_path=get_bench_path()) -> List:
...
def get_cmd_from_sysargv():
	...
	for arg in sys_argv:
		...
		if sys_argv.index(arg) == 0 and arg in Bench(get_bench_path()).apps:
			continue
