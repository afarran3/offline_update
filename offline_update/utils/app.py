...
from offline_update.exceptions import (
	InvalidRemoteException,
	InvalidBranchException,
	CommandFailedError,
	VersionNotFound,
)
from offline_update.app import get_repo_dir


def is_version_upgrade(app="frappe", bench_path=get_bench_path(), branch=None):
...
def switch_branch(branch, apps=None, bench_path=get_bench_path(), upgrade=False, check_upgrade=True):
	import git
	from offline_update.bench import Bench
	from offline_update.utils import log, exec_cmd
	from offline_update.utils.bench import (
		build_assets,
		patch_sites,
		post_upgrade,
	)
	from offline_update.utils.system import backup_all_sites
...
def switch_to_branch(branch=None, apps=None, bench_path=get_bench_path(), upgrade=False):
	switch_branch(branch, apps=apps, bench_path=bench_path, upgrade=upgrade)


def switch_to_develop(apps=None, bench_path=get_bench_path(), upgrade=True):
...
def get_develop_version(app, bench_path=get_bench_path()):
...
def get_upstream_version(app, branch=None, bench_path=get_bench_path()):
	repo_dir = get_repo_dir(app, bench_path=bench_path)
	if not branch:
...
def get_current_frappe_version(bench_path=get_bench_path()):
	try:
...
def get_current_branch(app, bench_path=get_bench_path()):
	from offline_update.utils import get_cmd_output
...
def get_remote(app, bench_path=get_bench_path()):
	repo_dir = get_repo_dir(app, bench_path=bench_path)
	contents = subprocess.check_output(
...
def get_current_version(app, bench_path=get_bench_path()):
