...
from offline_update.commands import bench_command
...
def cli():
	...
	bench_config = get_config(get_bench_path())
	...
	if (
		...
		and is_dist_editable(offline_update.PROJECT_NAME)
	):
		log(
...
def app_cmd(bench_path=get_bench_path()):
	f = get_env_cmd("python", bench_path=bench_path)
	os.chdir(os.path.join(bench_path, "sites"))
	os.execv(f, [f] + ["-m", "frappe.utils.bench_helper"] + sys.argv[1:])


def frappe_cmd(bench_path=get_bench_path()):
	f = get_env_cmd("python", bench_path=bench_path)
	os.chdir(os.path.join(bench_path, "sites"))
...
def get_frappe_help(bench_path=get_bench_path()):
...
def change_working_directory():
	...
	cur_dir = os.path.abspath(get_bench_path())
	bench_path = find_parent_bench(cur_dir)
	offline_update.current_path = os.getcwd()
	offline_update.updated_path = bench_path
