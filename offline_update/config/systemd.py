...
from offline_update.config.common_site_config import (
	get_gunicorn_workers,
	update_config,
	get_default_max_requests,
	compute_max_requests_jitter,
)
from offline_update.utils import exec_cmd, which, get_bench_name
...
def setup_main_config(bench_info, bench_path):
	# Main config
	bench_template = offline_update.config.env().get_template("systemd/frappe-bench.target")
...
def setup_workers_config(bench_info, bench_path):
	# Worker Group
	bench_workers_target_template = offline_update.config.env().get_template(
		"systemd/frappe-bench-workers.target"
	)
	bench_default_worker_template = offline_update.config.env().get_template(
		"systemd/frappe-bench-frappe-default-worker.service"
	)
	bench_short_worker_template = offline_update.config.env().get_template(
		"systemd/frappe-bench-frappe-short-worker.service"
	)
	bench_long_worker_template = offline_update.config.env().get_template(
		"systemd/frappe-bench-frappe-long-worker.service"
	)
	bench_schedule_worker_template = offline_update.config.env().get_template(
...
def setup_web_config(bench_info, bench_path):
	# Web Group
	bench_web_target_template = offline_update.config.env().get_template(
		"systemd/frappe-bench-web.target"
	)
	bench_web_service_template = offline_update.config.env().get_template(
		"systemd/frappe-bench-frappe-web.service"
	)
	bench_node_socketio_template = offline_update.config.env().get_template(
...
def setup_redis_config(bench_info, bench_path):
	# Redis Group
	bench_redis_target_template = offline_update.config.env().get_template(
		"systemd/frappe-bench-redis.target"
	)
	bench_redis_cache_template = offline_update.config.env().get_template(
		"systemd/frappe-bench-redis-cache.service"
	)
	bench_redis_queue_template = offline_update.config.env().get_template(
		"systemd/frappe-bench-redis-queue.service"
	)
	bench_redis_socketio_template = offline_update.config.env().get_template(
