...
import offline_update.config
from offline_update.bench import Bench
from offline_update.utils import get_bench_name
...
def make_nginx_conf(bench_path, yes=False, logging=None, log_format=None):
	...
	template = offline_update.config.env().get_template("nginx.conf")
...
def make_bench_manager_nginx_conf(bench_path, yes=False, port=23624, domain=None):
	from offline_update.config.site_config import get_site_config

	template = offline_update.config.env().get_template("bench_manager_nginx.conf")
	bench_path = os.path.abspath(bench_path)
	sites_path = os.path.join(bench_path, "sites")
...
def get_error_pages():
	bench_app_path = os.path.abspath(offline_update.__path__[0])
