...
def get_site_config(site, bench_path=get_bench_path()):
...
def put_site_config(site, config, bench_path=get_bench_path()):
	config_path = os.path.join(bench_path, "sites", site, "site_config.json")
	with open(config_path, "w") as f:
		return json.dump(config, f, indent=1)


def update_site_config(site, new_config, bench_path=get_bench_path()):
	config = get_site_config(site, bench_path=bench_path)
	config.update(new_config)
	put_site_config(site, config, bench_path=bench_path)


def set_nginx_port(site, port, bench_path=get_bench_path(), gen_config=True):
	set_site_config_nginx_property(
		site, {"nginx_port": port}, bench_path=bench_path, gen_config=gen_config
	)


def set_ssl_certificate(site, ssl_certificate, bench_path=get_bench_path(), gen_config=True):
	set_site_config_nginx_property(
		site,
...
def set_ssl_certificate_key(site, ssl_certificate_key, bench_path=get_bench_path(), gen_config=True):
	set_site_config_nginx_property(
		site,
...
def set_site_config_nginx_property(site, config, bench_path=get_bench_path(), gen_config=True):
	from offline_update.config.nginx import make_nginx_conf
...
def set_url_root(site, url_root, bench_path=get_bench_path()):
	update_site_config(site, {"host_name": url_root}, bench_path=bench_path)


def add_domain(site, domain, ssl_certificate, ssl_certificate_key, bench_path=get_bench_path()):
...
def remove_domain(site, domain, bench_path=get_bench_path()):
	domains = get_domains(site, bench_path)
...
def sync_domains(site, domains, bench_path=get_bench_path()):
	...
	if changed:
		...
		update_site_config(site, {"domains": domains}, bench_path=get_bench_path())

	return changed


def get_domains(site, bench_path=get_bench_path()):
