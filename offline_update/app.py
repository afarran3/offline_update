...
from offline_update.exceptions import NotInBenchDirectoryError
from offline_update.utils import (
	UNSET_ARG,
...
from offline_update.utils.bench import build_assets, install_python_dev_dependencies
from offline_update.utils.render import step

if typing.TYPE_CHECKING:
	from offline_update.bench import Bench


logger = logging.getLogger(offline_update.PROJECT_NAME)
...
class App(AppMeta):
	...
	def install(
		...
		from offline_update.utils.app import get_app_name
	...
	def _get_dependencies(self):
		from offline_update.utils.app import get_required_deps, required_apps_from_hooks
...
def get_excluded_apps(bench_path=get_bench_path()):
	try:
...
def write_excluded_apps_txt(apps, bench_path=get_bench_path()):
	with open(os.path.join(bench_path, "sites", "excluded_apps.txt"), "w") as f:
		return f.write("\n".join(apps))


def remove_from_excluded_apps_txt(app, bench_path=get_bench_path()):
	apps = get_excluded_apps(bench_path=bench_path)
	if app in apps:
		apps.remove(app)
		return write_excluded_apps_txt(apps, bench_path=bench_path)
...
def get_app(
	...
	git_url parameter.
	"""
	import offline_update as _bench
	import offline_update.cli as bench_cli
	from offline_update.bench import Bench
	from offline_update.utils.app import check_existing_dir
	...
	if not is_bench_directory(bench_path):
		...
		from offline_update.utils.system import init
...
def new_app(app, no_git=None, bench_path=get_bench_path()):
	if offline_update.FRAPPE_VERSION in (0, None):
		raise NotInBenchDirectoryError(
			f"{os.path.realpath(bench_path)} is not a valid bench directory."
		)
	...
	if no_git:
		if offline_update.FRAPPE_VERSION < 14:
...
def pull_apps(apps=None, bench_path=get_bench_path(), reset=False):
	"""Check all apps if there no local changes, pull"""
	from offline_update.bench import Bench
	from offline_update.utils.app import get_current_branch, get_remote
...
def get_repo_dir(app, bench_path=get_bench_path()):
...
def install_apps_from_path(path, bench_path=get_bench_path()):
