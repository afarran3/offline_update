...
from offline_update.exceptions import AppNotInstalledError, InvalidRemoteException
from offline_update.config.common_site_config import setup_config
...
from offline_update.utils.render import job, step
from offline_update.utils.app import get_current_version
from offline_update.app import is_git_repo


if TYPE_CHECKING:
	from offline_update.app import App
...
class Bench(Base, Validator):
	...
	@property
	def conf(self):
		from offline_update.config.common_site_config import get_config
...
class BenchSetup(Base):
	...
	def env(self, python="python3"):
		...
		verbose = offline_update.cli.verbose
	...
	def config(self, redis=True, procfile=True):
		...
		if redis:
			from offline_update.config.redis import generate_config

			generate_config(self.bench.name)

		if procfile:
			from offline_update.config.procfile import setup_procfile
	...
	def logging(self):
		from offline_update.utils import setup_logging
	...
	def python(self, apps=None):
		...
		quiet_flag = "" if offline_update.cli.verbose else "--quiet"
	...
	def node(self, apps=None):
		...
		from offline_update.utils.bench import update_node_packages
